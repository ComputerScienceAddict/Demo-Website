...
const Header = () => {
  ...
  const navLinks = ['Home', 'Rooms', 'Bookings', 'Restaurant', 'Spa', 'Contact'];
    ...
    >
      ...
      <div className='container mx-auto flex flex-col lg:flex-row items-center lg:justify-between gap-y-6 lg:gap-y-0'>
        ...
        flex gap-x-4 lg:gap-x-8 font-tertiary tracking-[3px] text-[15px] items-center uppercase`}>
          {
            navLinks.map(link => {
              const linkPath = link === 'Home' ? '/' : link === 'Bookings' ? '/bookings' : '/';
              return (
                <Link to={linkPath} className='transition hover:text-accent' key={link}>
                  {link}
                </Link>
              );
            })
